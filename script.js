...
async function sendForm(url, data, msgId){
  const msg = document.getElementById(msgId); msg.textContent='';
  try{
    ...
    if(res.ok){
      msg.textContent = json.message; msg.className='message';
  ...
  } catch(e){ msg.textContent='Ошибка соединения'; msg.className='error'; }
}
...
function initTabs(){
  ...
  tabs.forEach((t,i)=>{
    ...
    div.dataset.tab=t.tab; div.innerHTML=`<span>${t.name}</span>`;
    div.onclick=()=>{ document.querySelectorAll('.sidebar-item').forEach(s=>s.classList.remove('active')); div.classList.add('active'); selectTab(t.tab); };
...
function selectTab(tabName){
  ...
  main.innerHTML='';
  // Здесь будут функции renderGroupSummary, renderStudentSummary, renderStudentsTable и т.д.
  const h = document.createElement('h2'); h.innerText=`Вкладка: ${tabName}`; main.appendChild(h);
}
